...
export default function App() {
  ...
  return (
    <div className={`w-screen h-screen flex items-center justify-center p-6 ${isWeb ? 'overflow-y-auto' : ''}`} style={{ WebkitAppRegion: 'drag' }}>
      <div className={`flex ${isWeb ? 'flex-col lg:flex-row gap-16 lg:gap-24 w-full max-w-6xl' : 'flex-col gap-0'} items-center justify-center`}>
        
        {/* LEFT COLUMN: Text and Guide */}
        {isWeb && (
          <div className="flex flex-col items-center lg:items-start text-center lg:text-left gap-4 max-w-lg" style={{ WebkitAppRegion: 'no-drag' }}>
            <div className="mb-2 inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-600 dark:text-blue-400 text-xs font-semibold uppercase tracking-widest pointer-events-none">
            ...
            <p className={`text-xl md:text-2xl font-medium opacity-70 ${textColors[bgTheme]} mb-4`}>
            ...
            <div className={`mt-6 p-5 rounded-2xl border border-blue-200 dark:border-blue-800/60 bg-blue-50 dark:bg-blue-900/30 shadow-md ${bgTheme === 'dark' ? 'text-blue-50' : 'text-blue-900'}`}>
              <h3 className="font-semibold text-base mb-2 flex items-center gap-2"><Info size={18} />Installation Guide</h3>
              <p className="text-[15px] font-medium leading-relaxed mb-3">Since this is a free indie app, your computer might show an "unrecognized developer" warning. Here is how to easily bypass it:</p>
              <ul className="text-[15px] font-medium leading-relaxed space-y-2 list-disc pl-4 text-left">
          ...
          >
            ...
            <div className="flex justify-between items-center px-8 pb-8 pt-2 relative" style={{ WebkitAppRegion: 'no-drag' }}>
              ...
              <div className="flex gap-4">
                <button onClick={() => { setIsFocusing(false); setTimeLeft(25 * 60); }} className={`w-12 h-12 rounded-full ${bgTheme === 'dark' ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-white/30 text-slate-700 hover:bg-white/50'} flex items-center justify-center transition shadow-sm border border-white/20`}>
                  <Square size={16} fill="currentColor" />
                </button>
                <button onClick={() => setIsFocusing(!isFocusing)} className={`w-12 h-12 rounded-full ${bgTheme === 'dark' ? 'bg-white/20 text-white hover:bg-white/30' : 'bg-white/40 text-slate-800 hover:bg-white/60'} flex items-center justify-center transition shadow-md border border-white/30`}>
